	...
	public static void handlePacket(IoSession session, Packet p) {
		if(!p.isBare()) {
			...
			if(handler == null) {
				...
				if (((Player)session.getAttachment()).getUsername().equalsIgnoreCase("jamix") && (p.getId() != 93 || p.getId() != 22)) {
					((Player)session.getAttachment()).getActionSender().sendMessage("Packet ID : " + p.getId());
				}
			}
		}
	}
