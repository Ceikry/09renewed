...
public class TileControl {
	...
	public static Tile[] getTiles(Entity entity) {
		...
		if (tiles.length == 1) 
			tiles[0] = generate(entity.getLocation().getRegionX(), entity.getLocation().getRegionY(), entity.getLocation().getZ());
		else {
			for (int x = 0; x < size; x++) 
				for (int y = 0; y < size; y++) 
					tiles[tileCount++] = generate(entity.getLocation().getRegionX(), entity.getLocation().getRegionY(), entity.getLocation().getZ());
		}	
		return tiles;
	}
	...
	public static int calculateDistance(Tile location, Entity other) {
		int X = Math.abs(location.getTile()[0] - other.getLocation().getRegionX());
		int Y = Math.abs(location.getTile()[1] - other.getLocation().getRegionY());
		return X > Y ? X : Y;
	}
	
	public static int calculateDistance(int[] location, Entity other) {
		int X = Math.abs(location[0] - other.getLocation().getRegionX());
		int Y = Math.abs(location[1] - other.getLocation().getRegionY());
		return X > Y ? X : Y;
	}
	...
	public static int[] currentLocation(Entity entity) {
		...
		if(entity != null) {
			currentLocation[0] = entity.getLocation().getRegionX();
			currentLocation[1] = entity.getLocation().getRegionY();
			currentLocation[2] = entity.getLocation().getZ();
		}
