...
public class Prayer extends PrayerData {
	...
	public static void prayAtAltar(final Player p, int objectId, Location loc) {
		World.getInstance().registerCoordinateEvent(new AreaEvent(p, loc.getX() - 1, loc.getY() - 1, loc.getX() + 1, loc.getY() + 1) {

			@Override
			public void run() {
				if (p.getLevels().getLevel(5) < p.getLevels().getLevelForXp(5)) {
					p.animate(645);
					p.getLevels().setLevel(5, p.getLevels().getLevelForXp(5));
					p.getActionSender().sendSkillLevel(5);
					p.getActionSender().sendMessage("You recharge your prayer points.");
				}
			}
		});
	}
